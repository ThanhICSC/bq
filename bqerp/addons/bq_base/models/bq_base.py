...
class EmployeeTag(models.Model):
    _name = "bq.base.employee.tag"
    _description = "Employee Tag"
    _order = 'sequence, name'

    name = fields.Char(string="Name", required=True)
    color = fields.Integer(string='Color')
    employee_ids = fields.Many2many('bq.base.employee', 'bq_base_employee_tag_rel', 'tag_id', 'employee_id',
                                    string='Employees')
...
class Employee(models.Model):
    ...
    department_id = fields.Many2one('bq.base.department', string='Department', ondelete='restrict')
    other_department_ids = fields.Many2many('bq.base.department', string='Other Departments')
    ...
    note = fields.Text('Note')
    parent_id = fields.Many2one('bq.base.employee', string='Manager')
    child_ids = fields.One2many('bq.base.employee', 'parent_id', string='Subordinates')
    tag_ids = fields.Many2many('bq.base.employee.tag', 'bq_base_employee_tag_rel', 'employee_id', 'tag_id',
                               string='Tags')
    ...
    def _default_image(self):
        image_path = get_module_resource('bq_base', 'static/src/img', 'default_employee_image.png')
...
class Department(models.Model):
    ...
    company_id = fields.Many2one('res.company', string='Company', index=True,
                                 default=lambda self: self.env.user.company_id)
    parent_id = fields.Many2one('bq.base.department', string='Parent Department', index=True)
    child_ids = fields.One2many('bq.base.department', 'parent_id', string='Child Departments')
    manager_id = fields.Many2one('bq.base.employee', string='Manager')
    member_ids = fields.One2many('bq.base.employee', 'department_id', string='Members', readonly=True)
    note = fields.Text('Note')
...
class UserCreateWizard(models.TransientModel):
    _name = 'bq.base.user.create.wizard'
    ...
    employee_ids = fields.Many2many('bq.base.employee', string='Employees')
